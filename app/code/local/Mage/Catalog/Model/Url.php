...
class Mage_Catalog_Model_Url
{
    ...
    static protected $_refreshedProductRewrites = array();
    ...
    {
        if (!$this->_checkProductRewrite($product, $category)){
            return $this;
        }
    ...
    protected function _refreshCategoryProductRewrites(Varien_Object $category)
    {
        ...
        while ($process == true) {
            ...
            foreach ($products as $product) {
                ...
                if (Mage::getStoreConfig(Mage_Catalog_Helper_Product::XML_PATH_PRODUCT_URL_USE_CATEGORY, $category->getStoreId())){
    ...
    public function refreshProductRewrite($productId, $storeId = null)
    {
        ...
        if ($product) {
            ...
            if (Mage::getStoreConfig(Mage_Catalog_Helper_Product::XML_PATH_PRODUCT_URL_USE_CATEGORY, $storeId)){
    ...
    protected function _checkProductRewrite(Varien_Object $product, Varien_Object $category)
    {
        if (!isset(self::$_refreshedProductRewrites[$category->getStoreId()][$category->getId()][$product->getId()])){
            self::$_refreshedProductRewrites[$category->getStoreId()][$category->getId()][$product->getId()] = true;
            return true;
        }
        return false;
    }
