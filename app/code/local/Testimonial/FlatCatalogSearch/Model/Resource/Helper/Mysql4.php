 ...
 */
class Testimonial_FlatCatalogSearch_Model_Resource_Helper_Mysql4 extends Mage_Eav_Model_Resource_Helper_Mysql4
{
    ...
    function prepareTerms($str, $maxWordLength = 0)
    {
        ...
        if (strlen($str) && strlen($str) < 16){
            $codeNormalized = Mage::helper('magedoc')->normalizeCode($str);
            if (!preg_match('/[a-zA-Zà-ÿÀ-ß]{4,}/', $str)){
                $words = array($codeNormalized);
            } elseif ($codeNormalized != $str){
                $words[] = ">$codeNormalized";
            }
        }
