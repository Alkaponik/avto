 ...
 */
class Testimonial_FlatCatalogSearch_Helper_Data extends Mage_Core_Helper_Abstract
{
     ...
     * Query object
     *
     * @var Testimonial_FlatCatalogSearch_Model_Query
     */
    protected $_query;
     ...
     * Search engine model
     *
     * @var Testimonial_FlatCatalogSearch_Model_Resource_Fulltext_Engine
     */
    protected $_engine;
    ...
    {
        if (!$this->_query) {
            $this->_query = Mage::getModel('flatcatalogsearch/query')
     ...
     *
     * @return Testimonial_FlatCatalogSearch_Model_Resource_Query_Collection
     */
    ...
    {
        return $this->_getUrl('flatcatalogsearch/result', array(
    ...
    {
        return $this->_getUrl('flatcatalogsearch/ajax/suggest', array(
            '_secure' => Mage::app()->getFrontController()->getRequest()->isSecure()
        ));
    }
    ...
    {
        return $this->_getUrl('flatcatalogsearch/term/popular');
    }
    ...
    {
        return $this->_getUrl('flatcatalogsearch/advanced');
    }
    ...
    {
        return Mage::getStoreConfig(Testimonial_FlatCatalogSearch_Model_Query::XML_PATH_MIN_QUERY_LENGTH, $store);
    }
    ...
    {
        return Mage::getStoreConfig(Testimonial_FlatCatalogSearch_Model_Query::XML_PATH_MAX_QUERY_LENGTH, $store);
    }
    ...
    {
        return Mage::getStoreConfig(Testimonial_FlatCatalogSearch_Model_Query::XML_PATH_MAX_QUERY_WORDS, $store);
    }
     ...
     * @return Testimonial_FlatCatalogSearch_Helper_Data
     */
    ...
    public function checkNotes($store = null)
    {
        ...
        $searchType = Mage::getStoreConfig(Testimonial_FlatCatalogSearch_Model_Fulltext::XML_PATH_CATALOG_SEARCH_TYPE);
        if ($searchType == Testimonial_FlatCatalogSearch_Model_Fulltext::SEARCH_TYPE_COMBINE
            || $searchType == Testimonial_FlatCatalogSearch_Model_Fulltext::SEARCH_TYPE_LIKE
        ) {
    ...
    public function getEngine()
    {
        if (!$this->_engine) {
            ...
            if (!$this->_engine) {
                $this->_engine = Mage::getResourceSingleton('flatcatalogsearch/fulltext_engine');
            }
        }
    ...
    public function getCodeNormalized($searchString)
    {
        if(!preg_match('/[0-9]+/', $this->_searchString)){
            return false;
        }

        $productNumber = preg_replace('/[^a-zA-Z0-9\-\+=\/\.]*/', '', $searchString);
        $letters = preg_replace('/[^a-zA-Z]*/', '', $productNumber);
        if(strlen($letters) > self::MAX_LETTERS_COUNT_IN_NUMBER){
            $searchString = $this->_clearWords($searchString);
            $productNumber = preg_replace('/[^a-zA-Z0-9\-\+=\/\.]*/', '', $searchString);
        }

        $productNumberNormalized = preg_replace('/[^a-zA-Z0-9]*/', '', $productNumber);
        if (strlen($productNumberNormalized) < self::MIN_NUMBER_LENGTH){
            return false;
        }
        return $productNumberNormalized;
    }

    protected function _clearWords($searchString)
    {
        return preg_replace('/(\b[à-ÿÀ-ß]+\b)|(\b[a-zA-Z]{4,}\b)/', '', $searchString);
    }
}
