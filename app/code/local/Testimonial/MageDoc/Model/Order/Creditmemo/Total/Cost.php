...
class Testimonial_MageDoc_Model_Order_Creditmemo_Total_Cost extends Mage_Sales_Model_Order_Creditmemo_Total_Cost
{
    /**
    ...
    public function collect(Mage_Sales_Model_Order_Creditmemo $creditmemo)
    {
        ...
        foreach ($creditmemo->getAllItemsAndInquiries() as $item) {
            if (!$item->getHasChildren()){
                $baseRefundTotalCost += $item->getBaseCost()*$item->getQty();
            }
        }
        $creditmemo->setBaseCost($baseRefundTotalCost);
        $creditmemo->setCost($baseRefundTotalCost * $creditmemo->getBaseToGlobalRate());
        return $this;
    }
}
