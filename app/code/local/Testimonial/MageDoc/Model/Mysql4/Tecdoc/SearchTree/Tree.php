...
class Testimonial_MageDoc_Model_Mysql4_Tecdoc_SearchTree_Tree extends Varien_Data_Tree_Dbp
{
    const ID_FIELD    = 'STR_ID';
    const PATH_FIELD  = 'path';
    const ORDER_FIELD = 'STR_SORT';
    const LEVEL_FIELD = 'STR_LEVEL';
    ...
    protected $_additionalTable = null;
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            $resource->getConnection('magedoc_write'),
            Mage::getResourceModel('magedoc/tecdoc_searchTree')->getTable('magedoc/tecdoc_searchTree'),
            array(
                Varien_Data_Tree_Dbp::ID_FIELD       => 'STR_ID',
                Varien_Data_Tree_Dbp::PATH_FIELD     => 'path',
                Varien_Data_Tree_Dbp::ORDER_FIELD    => 'STR_SORT',
                Varien_Data_Tree_Dbp::LEVEL_FIELD    => 'STR_LEVEL',
            )
        );
        
        $this->_additionalTable = $resource->getTableName('magedoc/searchTree');
        
        $this->_select->joinInner($this->_additionalTable,
                "{$this->_table}.STR_ID = {$this->_additionalTable}.str_id", 
                'path');
        //$this->_select->columns(array('path' => new Zend_Db_Expr('CONCAT("1/", STR_ID)')));
    }
    
    /**
     * Enter description hermain_table...
     *
...
//            $collection->joinUrlRewrite();
    ...
    public function loadBreadcrumbsArray($path, $addCollectionData = true, $withRootNode = false)
    {
        ...
        if (!empty($pathIds)) {
           ...
           $select
                ->where('main_table.STR_ID IN(?)', $pathIds);
    ...
    protected function _createCollectionDataSelect($sorted = true, $optionalAttributes = array())
    {
        ...
        $collection = $this->_getDefaultCollection($sorted ? $this->_orderField : false);
        $collection->joinPath()
//                    ->setStoreId($storeId)
                    ->joinCategory()
                    ->joinDesignation($collection, 'main_table', 'STR_DES_ID', 'name');
    ...
    public function addChildNodes($children, $path, $parentNode, $level = 0)
    {
        ...
        return parent::addChildNodes($children, $path, $parentNode, $level);
    }
    ...
    public function load($parentNode=null, $recursionLevel = 0)
    {
        if (!$this->_loaded) {
            ...
            if ($parentPath) {
                $pathField = $this->_conn->quoteIdentifier(array($this->_additionalTable, $this->_pathField));
    ...
    public function loadNode($nodeId)
    {
        ...
        if (is_numeric($nodeId)) {
            $condField = $this->_conn->quoteIdentifier(sprintf('%s.%s',$this->_table,  $this->_idField));
        } else {
            $condField = $this->_conn->quoteIdentifier(sprintf('%s.%s',$this->_table, $this->_pathField));
        }
